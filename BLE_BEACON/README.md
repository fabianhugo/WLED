...
|---|---|
| Radio **on**, idle rainbow | ~70–120 mA |
| Radio **on**, paired peer near (pulsing, averages dimmer) | ~60–100 mA |
| Radio **off** (short press): BLE powered down, rainbow only | ~55–100 mA |

To go lower: reduce `MAX_BRIGHT` / `BRIGHTNESS` / `NUM_LEDS` (biggest win), drop the
scan duty cycle, or sleep the MCU while the radio is off (not implemented).
